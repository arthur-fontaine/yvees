...
char *ssid_wifi = "SSID_WIFI_REPLACE";            // Le nom du réseau WiFi
char *password_wifi = "PASSWORD_WIFI_REPLACE"; // Le password du WiFi

const char *mqtt_server = "IP_MQTT_1_REPLACE.IP_MQTT_2_REPLACE.IP_MQTT_3_REPLACE.IP_MQTT_4_REPLACE"; // L'IP de votre broker MQTT
const int mqtt_interval_ms = 5000;          // L'interval en ms entre deux envois de données

IPAddress localIP(192, 168, IP_CAR_3_REPLACE, IP_CAR_4_REPLACE); // l'IP que vous voulez donner à votre voiture

IPAddress localGateway(IP_GATEWAY_1_REPLACE, IP_GATEWAY_2_REPLACE, IP_GATEWAY_3_REPLACE, IP_GATEWAY_4_REPLACE); // L'IP de la gateway de votre réseau
IPAddress localSubnet(SUBNET_1_REPLACE, SUBNET_2_REPLACE, SUBNET_3_REPLACE, SUBNET_4_REPLACE);  // Le masque de sous réseau
...
void setup()
{
    ...
    // Make a sound to indicate that the car is ready
    Buzzer_Alarm(1);
    delay(100);
    Buzzer_Alarm(0);
}
